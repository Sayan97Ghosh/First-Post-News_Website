...
import Sports_Navbar from '../Components/Sports_Navbar';
import './Pages.css';
...
function Firstcricket() {
    const [userData,setUserData] = useState([]);
    useEffect(()=>{
      async function getUser() {
        try {
          const response = await axios.get(`https://hs-consumer-api.espncricinfo.com/v1/pages/story/news?lang=en&page=1`);
...
<BreadcrumbItem>
  <BreadcrumbLink href='#'>Cricket News</BreadcrumbLink>
</BreadcrumbItem>
</Breadcrumb>
      ...
      <hr style={{border:"1px solid gray",marginTop:"5%",marginBottom:"3%"}} />
  ...
  <Carousel/>
    ...
    {userData.map((ele)=>(
      <div key={ele.story.id}>
        <img src= {ele.story.imageId} alt="" />
         <div className='title'>
          <p>India</p>
          <span className='span1'></span>
          <h3 style={{fontWeight:"bolder"}}>{ele.story.seoHeadline}</h3>
          <p>{ele.story.coverImage.caption}</p>
         </div>
